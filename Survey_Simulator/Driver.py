...
while keep_going:
    ...
    # Draw an object 
    a,e,inc,node,peri,M,epoch,h,color,gb,ph,period,amp = go.GiMeObj(distri_file)

    # Write out the first 5000 objects to a file to give a small representative sample
    if n_iter <5000:
        drawn.write(str(a)+'\t'+str(e)+'\t'+str(inc/drad)+'\t'+str(h)+'\n')
    ...
    if flag > 0:
        ...
        ss.detWrite(detect_file, a, e, inc, r, mt, m_rand, h_rand, color, ic, flag, delta, m_int, h, eff, ra, dec, d_ra, d_dec, surna, comments)
        ...
        if (flag > 2) and (np.mod(flag,2) == 0):
            ...
            ss.trackWrite(track_file, a, e, inc, r, mt, m_rand, h_rand, color, ic, comments)
